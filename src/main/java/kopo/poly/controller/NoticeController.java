...
public class NoticeController {
    ...
    public String noticeList(HttpSession session, ModelMap model)
        ...
        // 교육용으로 아직 로그인을 구형하지 않았기 때문에 Session에 데이터를 저자아지 않았슴
        // 추후 로그인을 구현할 것으로 가정하고, 공지사항 리스트 출력하는 함수에서 로그인 한 것 처럼 Session 값을 새엉함
    ...
    public String noticeReg() {
        ...
        log.info("{}.noticeReg", this.getClass().getName());
    ...
    public MsgDTO noticeInsert(HttpServletRequest request, HttpSession session) {
        ...
        try {
             ...
             */
            log.info("session user_id: {}, title: {}, noticeYn {}, contents {}", session.getId(), title, noticeYn, contents);
    ...
    public String noticeInfo(@RequestParam @NonNull String nSeq, ModelMap model) throws Exception {
        ...
        /*
            결과값 전달은 반드시 DTO 객체를 이용해서 처리함 전달 받은 값을 DTO 객체에 넣는다.
         */
        NoticeDTO pDTO = NoticeDTO.builder().noticeSeq(Long.parseLong(nSeq)).build();

        // 공지사항 상세정보 가져오기
        // Kava 8부터 제공되는 Optional 활용하여 NPE(Null Pointer Exception) 처리
    ...
    public String noticeEditInfo(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        /*
            값 전달은 반드시 DTO 객체를 이용해서 처리함 전달 받안 값을 DTO 객체에 넣는다.
         */
        NoticeDTO pDTO = NoticeDTO.builder().noticeSeq(Long.parseLong(nSeq)).build();
    ...
    public MsgDTO noticeUpdate(HttpSession session, HttpServletRequest request) {
        ...
        try {
            ...
            /*
                값 전달은 받으시 DTO 객체를 이용해서 처리함 전달 받은 값을 DTO 객체에 넣는다.
             */
