...
public class NoticeJoinController {
    ...
    public String noticeListUsingJoinColumn(HttpSession session, ModelMap model)
        ...
        log.info(this.getClass().getName() + ".noticeListJoinColumn Start!");
        ...
        log.info(this.getClass().getName() + ".noticeListJoinColumn End!");

        // 함수 처리가 끝나고 보여줄 HTML (Thymeleaf) 파일면
        // templates/notice/noticeList.html
        return "notice/noticeListJoin";
    }

    @GetMapping(value = "noticeListUsingNativeQuery")
    public String noticeListUsingJoinQuery(HttpSession session, ModelMap model)
            throws Exception {

        // 로그 찍기(추후 찍은 로그를 통해 이 함수에 접근했는지 파악하기 용이하다.)
        log.info(this.getClass().getName() + ".noticeListNativeQuery Start!");
        ...
        // 로그 찍기(추후 찍은 로그를 통해 이 함수 호출이 끝났는지 파악하기 용이하다.)
        log.info(this.getClass().getName() + ".noticeListNativeQuery End!");
    ...
    public String noticeListUsingQueryDSL(HttpSession session, ModelMap model)
        ...
        // 로그 찍기(추후 찍은 로그를 통해 이 함수에 접근했는지 파악하기 용이하다.)
        log.info(this.getClass().getName() + ".noticeListUsingQueryDSLStart!");
