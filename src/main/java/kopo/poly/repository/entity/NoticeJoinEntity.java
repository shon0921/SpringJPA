...
public class NoticeJoinEntity {
    ...
    /*
        ...
        name => NoticeJoinEntity 조인할 칼럼명 (즉, Notice 테이블의 user_id 칼럼과 User_info 테이블 조인)
        ...
        insertable = false 칼럼에 등록 막기
        updatable = false 칼럼에 수정 막기
        <p>
        변수 타입에 UserInfoEntity 정의하면, 자동으로 User_info 테이블이 조인 대상이 됨
        외래키는 PK 값과 연결되는 구조이기에 user_info 테이블의 PK 칼럼인 user_id가 조인 대상이 됨
        즉, user_info 테이블의 user_id 칼럼과 notice 테이블의 user_id 칼럼이 조인됨
        <p>
        만약, user_info 테이블의 PK 칼럼이 아닌 user_name과 같이 다른 칼럼 사용하고 싶은 경우
